...
vector<string> robot(const string &s,const vector<vector<vector<string>>> &dir_path){
    unordered_map<char,int> m;
    m['^']=0,m['<']=1,m['v']=2,m['>']=3,m['A']=4;

    int prev=m['A'],nxt=m[s[0]];

    vector<string> arr=dir_path[prev][nxt];
    prev=nxt;
...
int main(){
    ...
    for(int i=0;i<5;i++){
        ...
        int temp=0,maxi=INT_MAX;
        vector<string> arr=numpad(s,num_path);
        
        for(string v:arr){
            vector<string> brr=robot(v,dir_path);
            for(string w:brr){
                vector<string> crr=robot(w,dir_path);
                for(string x:crr){
                    if(x.size()<maxi) temp=res*(x.size()),maxi=x.size();
                }
            }
        }
        ans+=temp;
